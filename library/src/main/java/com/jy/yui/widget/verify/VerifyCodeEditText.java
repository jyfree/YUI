package com.jy.yui.widget.verify;
...
public class VerifyCodeEditText extends RelativeLayout {
    ...
    private void init(Context context, AttributeSet attrs, int defStyleAttr) {
        LayoutInflater.from(context).inflate(R.layout.yui_layout_verify_code, this);
        ...
        mEtNumber = typedArray.getInteger(R.styleable.VerifyCodeEditText_vci_number, 4);
        mEtWidth = typedArray.getDimensionPixelSize(R.styleable.VerifyCodeEditText_vci_width, YUIHelper.INSTANCE.dip2px(21));
        int dividerResourceId = typedArray.getResourceId(R.styleable.VerifyCodeEditText_vci_divider, 0);
        if (dividerResourceId != 0) {
            mEtDivider = ContextCompat.getDrawable(getContext(), dividerResourceId);
        }
        mEtTextSize = typedArray.getDimension(R.styleable.VerifyCodeEditText_vci_text_size, YUIHelper.INSTANCE.sp2px(16f));
        mEtTextColor = typedArray.getColor(R.styleable.VerifyCodeEditText_vci_text_color, Color.BLACK);
        int focusBGResourceId = typedArray.getResourceId(R.styleable.VerifyCodeEditText_vci_bg_focus, 0);
        if (focusBGResourceId != 0) {
            mBackgroundFocus = ContextCompat.getDrawable(getContext(), focusBGResourceId);
        }
        int normalBGResourceId = typedArray.getResourceId(R.styleable.VerifyCodeEditText_vci_bg_normal, 0);
        if (normalBGResourceId != 0) {
            mBackgroundNormal = ContextCompat.getDrawable(getContext(), normalBGResourceId);
        }
        mIsPwd = typedArray.getBoolean(R.styleable.VerifyCodeEditText_vci_is_pwd, false);
        mPwdRadius = typedArray.getDimensionPixelSize(R.styleable.VerifyCodeEditText_vci_pwd_radius, YUIHelper.INSTANCE.dip2px(5));
        //释放资源
        typedArray.recycle();

        // 当xml中未配置时 这里进行初始配置默认图片
        if (mEtDivider == null) {
            mEtDivider = YUIHelper.INSTANCE.getDrawableRes(R.drawable.yui_vci_shape_divider);
        }
        if (mBackgroundFocus == null) {
            mBackgroundFocus = YUIHelper.INSTANCE.getDrawableRes(R.drawable.yui_vci_shape_bg_focus);
        }
        if (mBackgroundNormal == null) {
            mBackgroundNormal = YUIHelper.INSTANCE.getDrawableRes(R.drawable.yui_vci_shape_bg_normal);
        }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (heightMode == MeasureSpec.AT_MOST) {
            heightMeasureSpecValue = MeasureSpec.makeMeasureSpec(YUIHelper.INSTANCE.dip2px(50), MeasureSpec.EXACTLY);
        }
    ...
    private void setListener() {
        ...
        mEditText.setBackSpaceListener(new InputIntercept.BackspaceListener() {
            @Override
